...
namespace BC7.Business.Implementation.Tests.Integration.Tests.UpgradingMatrix
{
    [Story(
        ...
        IWant = "I want to upgrade my matrix position to the higher level and my sponsor did the upgrade before me",
        SoThat = "So I will have position in a higher matrix level in the same structure position as before - in the nearest empty position under my sponsor"
    )]
    public class UpgradeMatrixAsNormalUserWhenSponsorDidUpgradeBeforeTest : BaseIntegration
    {
        private UpgradeMatrixCommandHandler _sut;
        private UpgradeMatrixCommand _command;
        private UpgradeMatrixResult _result;

        private readonly Guid _sponsorMultiAccountId = Guid.Parse("614b3ca8-486e-480d-b552-5e7eacc1f8ef");
        ...
        async Task AndGivenCreatedDefaultAccountsAndMatricesInDatabase()
        {
            ...
            var sponsorUser = new UserAccountData(
                id: Guid.NewGuid(),
                login: "SponsorLogin",
                email: "SponsorEmail",
                firstName: "SponsorFirstName",
                lastName: "SponsorLastName",
                street: "SponsorStreet",
                city: "SponsorCity",
                country: "SponsorCountry",
                zipCode: "SponsorZipCode",
                btcWalletAddress: "SponsorBtcWalletAddress",
                role: UserRolesHelper.User
            );
            sponsorUser.SetPassword("salt", "hash");
            sponsorUser.PaidMembershipFee();

            var user1 = new UserAccountData(
                id: Guid.NewGuid(),
                login: "User1Login",
                email: "User1Email",
                ...
                btcWalletAddress: "User1BtcWalletAddress",
                role: UserRolesHelper.User
            );
            user1.SetPassword("salt", "hash");
            user1.PaidMembershipFee();

            var testUser = new UserAccountData(
                id: Guid.NewGuid(),
                login: "TestLogin",
                email: "TestEmail",
                ...
                country: "TestCountry",
                zipCode: "TestZipCode",
                btcWalletAddress: "TestBtcWalletAddress",
                role: UserRolesHelper.User
            );
            testUser.SetPassword("salt", "hash");
            testUser.PaidMembershipFee();
        ...
        async Task AndUserHasMatrixPositionOnUpgradedLevelUnderSponsor()
        {
